"""
Note: Py3 variation/refactor of https://github.com/Botffy/polyskel
      Modification by Kuan Butts (kuanbutts@gmail.com) Sep 01, 2017

Original description:
Implementation of the straight skeleton algorithm as described by
Felkel and Obdržálek in their 1998 conference paper Straight
...
from ft_euclid import *
import importlib
# importlib.reload(ft_euclid)
...
_OriginalEdge = namedtuple('_OriginalEdge',
                           'edge bisector_left, bisector_right')
...
class _LAVertex(abc.ABC):
    def __init__(self, point, edge_left, edge_right, direction_vectors=None):
        ...
        # TODO: Handle validity better; maybe membership in LAV implies true?
        self._valid = True;

        creator_vectors = ((edge_left.v.normalized() * -1),
        ...
        self._bisector = (Ray2(self.point, operator.add(*creator_vectors) * 
                          (-1 if self.is_reflex else 1)))
        
        log.info("Created vertex %s", self.__repr__())
        _debug.line((self.bisector.p.x,
                     self.bisector.p.y,
    ...
    def next_event(self):
        events = []
        if self.is_reflex:
            # Note: A reflex vertex may generate a split event
            # Note: Split events happen when a vertex hits an opposite edge,
            #       splitting the polygon in two.
            log.debug("looking for split candidates for vertex %s", self)
            
            for edge in self.original_edges:
                if ((edge.edge == self.edge_left) or
                    (edge.edge == self.edge_right)):
                    continue

                log.debug("\tconsidering EDGE %s", edge)

                # Note: A potential b is at the intersection of between our 
                #       own bisector and the bisector of the angle
                #       between the tested edge and any one of our own edges.

                # Note: We choose the 'less parallel' edge (in order to
                #       exclude a potentially parallel edge)
                ...
                if i is not None and not _approximately_equals(i, self.point):
                    ...
                    # Note: A valid b should lie within the area limited by
                    #       the edge and the bisectors of its two vertices:
                    ...
                    split_event_l2 = _SplitEvent(Line2(edge.edge).distance(b),
                                                 b, self, edge.edge)
                    events.append(split_event_l2)

        i_prev = self.bisector.intersect(self.prev.bisector)
        i_next = self.bisector.intersect(self.next.bisector)

        if i_prev is not None:
            events.append(_EdgeEvent(Line2(self.edge_left).distance(i_prev),
                          i_prev, self.prev, self))
        if i_next is not None:
            events.append(_EdgeEvent(Line2(self.edge_right).distance(i_next),
                          i_next, self, self.next))
